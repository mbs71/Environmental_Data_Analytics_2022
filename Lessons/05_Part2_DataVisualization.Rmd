...
* axis label colors - default is black and grey
...
theme_set(mytheme) #set theme for all subsequetn plots
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #nice when you don't want gridlines everywhere but you want one at specific numbers
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + # date breaks changes it to show a lavel for every 2 months, %b gives abbriviated 3 letters of month name and %y gives just two digets for year
  ylab(expression("O"[3]* " AQI Value")) + #change y label
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #change angle of x axis text
print(O3plot7)  
...
library(cowplot)
plot_grid(NvsP2, Nutrientplot, nrow = 2, align = 'h', rel_heights = c(1.25, 1)) #NVSP@ and Nutrientplot are two plots we made earlier. Rel height gives the respective height alloted to each plot 
