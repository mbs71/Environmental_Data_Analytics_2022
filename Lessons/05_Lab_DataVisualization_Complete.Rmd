...
print(Plot_text) #adding lavel at a specific point 
...
ggplot(filter(PeterPaul.chem.nutrients, depth==0))+
  geom_point(aes(x=daynum, y=temperature_C, color=year4))+ #in simple plots it doesn't matter if aes is with geom instead of ggplot. If you put your aes with geom you have more control because if you wanted to add lines you.
  facetwrap('lakename')+
  ylab('Temperature(C)')
...
ggplot(PeterPaul.chem.nutrients, depth==0) + 
  geom_point(aes(x=sampledate, y=temperature_C, color=depth, size=0.5))+
  scale_color_viridis(option = 'magma', direction = -1)+ #direction flips the coloring
  scale_x_date(date_breaks = "5 years", date_labels = '%Y')
#scale_x_date we're making it every five years and changing the label to two digit years with the capital y 
...
# Make the points 50 % transparent -> use alpha for this
...
#note year is imported as something but we want it to be a cetogry so use as.factor
...
#Solution
#The reorder statement appends info on the Endpoint vector, listing values
#as factors ordered by the number of records (from the 'length' function)

#Show structure of raw imported values 
str(Neonics$Endpoint)
#Show structure of raw imported values, converted to factors
str(as.factor(Neonics$Endpoint))
#Reorder the raw values 
