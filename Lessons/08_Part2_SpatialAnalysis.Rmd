...
huc8_UpperNeuse_sf<- st_read('./Data/Spatial/NCHUC8.shp') %>% 
...
mapView(
  huc8_sf, col.regions = 'orange', color = 'orange', alpha = 1)+
  mapview(
    huc8_UpperNeuse_sf, col.regions = 'purple', color = 'purple', alpha = 1)



ggplot()+
  geom_sf(data=huc8_sf, fill = "orange")+
  geom_sf(data=huc8_UpperNeuse_sf, fill = "purple")
...
#supply col classes with a vector (FIPS) and column name (factor)
...
#selecting row filter as triangle county
traingle_HUCs <- huc8_sf_utm[triangle_co,]
...
triangle_HUCs.clipped <- traingle_co %>%
