...
>The p value is lower than alpha (0.05) so there is meaningful correlation here  

So, we see there is a significant negative correlation between irradiance and depth (lower light levels at greater depths), and that this model explains about 31 % (r-sqaured value) of the total variance in irradiance. Let's visualize this relationship and the model itself. 
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #changing plotting window to hide outlier and get a better look at the rest of the data
  geom_point() 
