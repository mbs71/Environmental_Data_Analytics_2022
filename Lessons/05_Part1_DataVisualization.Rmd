...
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) this added a legend but did not change the color
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #creating one plot for each site
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #columns representing year and rows are the sites
print(PMplot.faceted2)

# Plot true time series with geom_line
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) + #change x axis for line plot to date so its a time series
  geom_line()
print(PMplot.line)
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# Scatterplot
# this is not a nice visualization because two of the points are so far from the others. You should adjust the axex - see the next code line 138
lightvsDO <- 
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #if you don't specify thisyour values will be plotted from 0 to waterever but because of how water data is you want zero on top because it is the water's surface
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #lm is a linear model line of fit
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Fill changes just the color inside the box so the color command looks beter
print(Nutrientplot3)
...
print(Nutrientplot4)
#Position = dodge makes sure no dots are overlayed on one another
...
#Problems are that there is no legend with this an easier way to do this is the pivot wider and pivot longer, see line 201
Nutrientplot6 <-
...
# Ridgeline plot; categorizing y by nutrients and the fill rather than color funciton looks nice here
Nutrientplot6 <-
