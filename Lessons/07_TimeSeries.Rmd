...
Alternative way to code lines 117-119
EnoDischarge.late_ts <- ts(EnoDischarge.late[[8]], start = c(1985,1), frequency = 365)
frequency 365 lets the code know you are working with daily data 
...
#zoo package and na.approx function which is used to interpolate using linear or spline interpolation. 
...
* **Augmented Dickey Fuller**: no seasonality, non-parametric, missing data not allowed. Function: `adf.test()` (package: tseries). For stochastic trend not monotonic trend.
...
> Answer: There is a decreased trend of the average wind speed overtime.
