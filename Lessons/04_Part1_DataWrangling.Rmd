...
Here are the relevant commands used in the `filter` function. Add some notes to designate what these commands, called mean. 
`==` check for values that are equal or the same
`!=` check for values that are different 
`<` check for values less than greater than or greater than and eaul
`<=`
`>`
`>=`
`&` and
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #filtering the rows that have the elements 0 on column depth

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)#data, column, value that you want filtered
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #all instances of paul lake OR peter lake
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #the comma is the same as &
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) # data set, the column you want to be arranged
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth))#data set, command for descending order (column you want arranged)
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #making a smaller data frame by choosing the columns you want included
...
```{r}
#adding a new column that is some variation of an already existing column
...
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) # when you data set ordered by date
